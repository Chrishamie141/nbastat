...
def _save_nba_parlay_history(parlay):
    legs = [
        ParlayLeg(
            sport=SportType.NBA,
            ...
            line=leg.get("line"),
            odds=leg.get("sportsbook_odds"),
            prediction=f"{leg.get('player')} over {leg.get('line')} {leg.get('stat_type')}",
            confidence=float(leg.get("model_probability", 0) or 0) * 100,
            notes=f"Edge: {float(leg.get('edge', 0) or 0) * 100:+.1f}%",
        )
    ...
    result = ParlayResult(
        parlay=Parlay(
            sport=SportType.NBA,
            difficulty=DifficultyLevel.from_input(parlay.get("style")),
            legs=legs,
            notes="NBA parlay generated by existing betting engine.",
        ),
        estimated_odds=parlay.get("estimated_odds"),
        combined_probability=parlay.get("adjusted_combined_probability"),
...
def run_auto_parlay_mode(season="2025-26"):
    ...
    parlay_id = _save_nba_parlay_history(parlay)
    print(f"\nSaved {len(recommendations)} bet recommendations and parlay history row #{parlay_id} to predictions.db.")
...
def run_parlay_history_mode():
    print("\nParlay History")
    sport = input("Filter sport (NFL/NBA, optional): ").strip().upper()
    difficulty = input("Filter difficulty (SAFE/BALANCED/AGGRESSIVE, optional): ").strip().upper()
    result_status = input("Filter result status (pending/hit/missed, optional): ").strip().lower()
    rows = load_parlay_history(sport=sport or None, difficulty=difficulty or None, result_status=result_status or None)
    if not rows:
        print("No matching parlay history found.")
        return
    for row in rows:
        print(f"\n#{row['id']} {row['created_at']} | {row['sport']} | {row['difficulty']} | {row['result_status']}")
        print(f"Odds: {row.get('estimated_odds') or 'N/A'} | Combined probability: {row.get('combined_probability') or 0:.3f}")
        print(row.get("legs_json", "[]"))
        if row.get("notes"):
            print(f"Notes: {row['notes']}")


def print_sport_menu():
    print("Multi-Sport Betting/Parlay Tool")
    print("1. NFL")
    print("2. NBA")
...
def main(argv=None):
    ...
    run_health_check()
    print_sport_menu()
    sport_choice = input("Select sport 1 or 2: ").strip() or "2"
    if sport_choice == "1":
        print("\nNFL Menu")
        ...
        print("5. View NFL Performance Report")
        nfl_mode = input("Select mode 1, 2, 3, 4, or 5: ").strip()
        if nfl_mode == "2":
            show_fantasy_menu()
        elif nfl_mode == "3":
