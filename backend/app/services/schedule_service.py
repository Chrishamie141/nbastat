...
def _team(comp):
    ...
    return TeamSummary(id=str(t.get('id') or t.get('abbreviation') or ''), name=t.get('displayName') or t.get('name') or t.get('abbreviation') or 'TBD', abbreviation=t.get('abbreviation') or 'TBD', logoUrl=(logos[0].get('href') if logos else None), record=None)
...
def upcoming_games(leagues, limit=10, start=None, end=None):
    ...
    for lg in leagues:
        ...
        for ev in events:
            ...
            d={'id':str(ev.get('id')), 'league':lg, 'seasonPhase':_phase(lg,dt), 'awayTeam':_team(away), 'homeTeam':_team(home), 'startTimeUtc':dt, 'status':'scheduled' if 'pre' in status or 'scheduled' in status else status, 'venue':(comp.get('venue') or {}).get('fullName'), 'broadcast':broadcasts, 'nationalBroadcast':bool(broadcasts), 'dataProvider':provider, 'dataMode':'live'}
