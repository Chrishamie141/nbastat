...
def _initialize_predictions_for_database(_target_url: str) -> None:
...
def _initialize_predictions() -> None:
    # Schema setup and legacy migration are process-scoped for each database.
    # Request paths can call this safely without repeating DDL for every game.
    _initialize_predictions_for_database(database_url())


_initialize_predictions.cache_clear = _initialize_predictions_for_database.cache_clear
...
def _save_prediction_with_connection(connection, user_id: int, game: dict, prediction: dict) -> None:
...
def _save_prediction(user_id: int, game: dict, prediction: dict) -> None:
    _initialize_predictions()
    with get_db_connection() as connection:
        _save_prediction_with_connection(connection, user_id, game, prediction)


def _save_predictions(rows: list[tuple[int, dict, dict]]) -> None:
    if not rows:
        return
    _initialize_predictions()
    with get_db_connection() as connection:
        for user_id, game, prediction in rows:
            _save_prediction_with_connection(connection, user_id, game, prediction)
...
def weekly_board(season: int, week: int, profile: str, user_id: int, day: str | None = None,
    ...
    season_type = _season_type(season_type)
    schedule = []
    for raw_game in _schedule(season, week, season_type):
        game = {**raw_game}
    ...
    schedule_ms = (perf_counter() - started) * 1000
    ...
    system_snapshots = (user_snapshots if user_id == 0 else
                        _prediction_snapshots(0, season, week, season_type))
    snapshots = {**user_snapshots, **system_snapshots}
    snapshot_ms = (perf_counter() - snapshot_started) * 1000
    ...
    for game in schedule:
        market_history = market_histories[game["game_id"]]
        ...
        if projection_data:
            ...
            item["marketHistory"] = market_history
            pending_predictions.append((0, game, prediction))
            if user_id != 0:
                pending_predictions.append((user_id, game, prediction))
        else:
    ...
    if pending_predictions:
        _save_predictions(pending_predictions)
    ...
    result = {"season": season, "week": week, "displayWeek": week, **identity, "profile": profile, "items": items,
    ...
    logger.info(
        "nfl_weekly_board_complete season=%s phase=%s week=%s games=%s schedule_ms=%.1f "
        "snapshots_ms=%.1f slate_database_ms=%.1f total_ms=%.1f",
        season, season_type, week, len(items), schedule_ms, snapshot_ms, database_ms,
...
def _persisted_current_week_context(season: int, now: datetime) -> dict | None:
    """Read the active automation slate without contacting schedule providers."""
    try:
        with get_db_connection() as connection:
            if not table_exists(connection, "nfl_automation_state"):
                return None
            row = connection.execute(
                """SELECT e.season,e.season_type,e.display_week,e.provider_week,e.status,
                MIN(g.kickoff_epoch) AS first_kickoff_epoch,
                MAX(g.kickoff_epoch) AS last_kickoff_epoch
                FROM nfl_automation_state s
                JOIN nfl_capture_experiments e ON e.experiment_key=s.active_experiment_key
                JOIN nfl_capture_games g ON g.experiment_key=e.experiment_key
                WHERE s.singleton=1 AND e.season=? AND e.status='ACTIVE'
                GROUP BY e.season,e.season_type,e.display_week,e.provider_week,e.status""",
                (season,),
            ).fetchone()
    except Exception as exc:
        logger.warning("nfl_context_persisted_lookup_failed code=%s", type(exc).__name__)
        return None
    if not row or row["last_kickoff_epoch"] is None:
        return None
    # The automation seals a slate eight hours after its last kickoff. Ignore a
    # stale active pointer beyond the same boundary and provider-verify instead.
    if now.timestamp() > float(row["last_kickoff_epoch"]) + 8 * 3600:
        return None
    season_type, week = str(row["season_type"]), int(row["display_week"])
    postseason_labels = {
        1: "Wild Card", 2: "Divisional Round", 3: "Conference Championships",
        4: "Pro Bowl", 5: "Super Bowl",
    }
    week_key = ("HOF" if season_type == "preseason" and week == 0 else
                f"PRE{week}" if season_type == "preseason" else
                f"POST{week}" if season_type == "postseason" else f"REG{week}")
    week_label = ("Hall of Fame Game" if week_key == "HOF" else
                  f"Preseason Week {week}" if season_type == "preseason" else
                  postseason_labels.get(week, f"Postseason Week {week}")
                  if season_type == "postseason" else f"Week {week}")
    return {
        "season": int(row["season"]), "seasonType": season_type, "week": week,
        "displayWeek": week, "providerWeek": int(row["provider_week"]),
        "weekKey": week_key, "weekLabel": week_label,
        "hasUpcoming": float(row["last_kickoff_epoch"]) >= now.timestamp(),
...
def current_week_context(season: int, at: datetime | None = None) -> dict:
    ...
    if persisted:
        return persisted
    # The calendar-guided fallback verifies only the nearby candidate weeks
    # (two or three during the regular season), never all 29 possible slates.
    from backend.app.services.nfl_server_automation import resolve_active_context

    return {
        **resolve_active_context(season, at=now, schedule_loader=_schedule),
