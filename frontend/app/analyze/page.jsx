...
function Flow(){
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-4 pb-28 pt-10 sm:px-6 md:pt-14">
    ...
    {sm?.mode==='offseason'&&step!==4?<GlowCard className="p-6"><h2 className="text-2xl font-bold">Supported analysis is paused</h2><p className="mt-3 text-slate-300">New NFL and NBA analysis is paused while both leagues are outside the supported window. Previous activity remains available.</p><div className="mt-5 flex gap-3"><Link className="btn btn-glass" href="/history">History</Link><Link className="btn btn-glass" href="/performance">Performance</Link></div></GlowCard>:<AnimatePresence mode="wait"><motion.div key={`${step}-${action}`} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-12}}>
      ...
      {step===3&&sport==='NBA'&&<NBAOptions action={action} opts={opts} setOpts={setOpts} loading={loading} error={error} run={runNBA} back={()=>setStep(2)} restart={restart}/>}
...
function NBAOptions({action,opts,setOpts,loading,error,run,back,restart}){return <GlowCard className="mx-auto max-w-3xl p-6"><h2 className="text-2xl font-bold">{action}</h2><div className="mt-5 grid gap-5">{action.includes('Single')&&<input required aria-label="NBA player name" placeholder="NBA player name" value={opts.player} onChange={event=>setOpts({...opts,player:event.target.value})} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3"/>}{action.includes('Team')&&<TeamSelector league="nba" value={opts.team} onChange={team=>setOpts({...opts,team})}/>} {action.includes('Default')&&<p className="rounded-2xl bg-white/5 p-4 text-gray-300">This analysis uses the configured application roster.</p>}</div>{error&&<p className="mt-4 rounded-xl bg-red-500/10 p-3 text-red-200">{error}</p>}<div className="mt-6 flex flex-wrap gap-3 border-t border-white/10 pt-5"><button onClick={back} className="btn btn-glass">Back</button><button disabled={loading} onClick={run} className="btn btn-primary">{loading?'Running analysis…':'Run Analysis'}</button><button onClick={restart} className="btn btn-glass">Restart</button></div></GlowCard>}
