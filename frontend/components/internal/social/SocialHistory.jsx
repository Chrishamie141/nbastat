...
export default function SocialHistory({ items = [], busy = false, onRetry }) {
  return <GlowCard className="p-6">
    ...
    <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm">
      ...
      <tbody>{items.map((post) => <tr key={post.post_id} className="border-t border-white/10">
        ...
        <td className="max-w-sm py-4 pr-5 text-slate-300"><details><summary className="cursor-pointer line-clamp-2">{post.content}</summary><p className="mt-2 whitespace-pre-wrap text-xs leading-5">{post.content}</p></details></td>
        ...
        <td>{post.xUrl ? <><a href={post.xUrl} target="_blank" rel="noreferrer" className="font-bold text-cyan-300">Open live post</a><p className="mt-1 font-mono text-[10px] text-slate-500">{post.x_post_id}</p></> : post.status === "FAILED" && onRetry ? <button disabled={busy} onClick={() => onRetry(post.post_id)} className="btn btn-glass px-3 py-2 text-xs">Retry safe failure</button> : EMPTY}</td>
      </tr>)}</tbody>
    </table></div>
  </GlowCard>;
}
