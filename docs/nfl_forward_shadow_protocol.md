...
`python -m backtesting.capture_nfl_live_player_props season-plan --snapshot-root backtesting/data/snapshots --season 2026 --start-week 1 --end-week 18 --as-of TIMESTAMP --output REPORT` aggregates the weekly checks without network access. It records the next capture-window opening, identity-ready weeks, currently eligible games, and the current maximum credit exposure. A future theoretical season total is intentionally not an authorization; only games ready at the supplied timestamp contribute to the current ceiling.
